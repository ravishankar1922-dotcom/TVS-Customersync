...
export default function Dashboard({ onNavigate }) {
  ...
  async function triggerEmails() {
    ...
    try {
      ...
      setEmailResult(r);
      toast(r.smtp_configured ? `${r.sent} emails sent` : `SMTP not configured — ${r.results.length} portal links generated (see Email Log)`, 'info', 5000);
      load();
    } catch (e) { toast(e.message, 'err'); }
    ...
    <div>
      <div className="sec-hd">
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          <button className="btn btn-secondary btn-sm" onClick={() => onNavigate('ledger')}><Icon name="upload" size={13} /> Upload Ledger</button>
          <button className="btn btn-secondary btn-sm" onClick={() => setEmailModal(true)}><Icon name="mailLog" size={13} /> Email Log</button>
          ...
          <button className="btn btn-primary" onClick={triggerEmails} disabled={triggering}>
            {triggering ? <><Spinner /> Triggering…</> : <><Icon name="mail" size={13} /> Trigger Customer Emails</>}
          </button>
        ...
        <>
                ...
                {pageData.map(c => {
                  ...
                  return (
                    <tr key={c.customer_id}>
                      ...
                      <td>
                        {c.soa_file ? <a href={api.soaDownloadUrl(c.customer_id)} className="btn btn-ghost btn-sm" style={{ fontSize: 10 }}><Icon name="download" size={11} /> Download</a> : <span style={{ color: 'var(--muted-lt)', fontSize: 10 }}>—</span>}
                        {c.reupload_status === 'REQUESTED' && <div style={{ marginTop: 4 }}><span className="badge b-diff" style={{ fontSize: 9 }} title="Customer has requested to re-upload their SOA"><Icon name="refresh" size={10} /> Re-upload requested</span></div>}
                      </td>
                      <td><span style={{ fontSize: 10, color: 'var(--muted)' }}>{c.token_status?.replace('_', ' ')}</span></td>
                      <td>
                        <div style={{ display: 'flex', gap: 4 }}>
                          <button className="act-btn" title="Details" onClick={() => setDetailModal(c)}><Icon name="eye" size={14} /></button>
                          <button className="act-btn" title="Preview Email" onClick={() => showPreview(c.customer_id)}><Icon name="mail" size={14} /></button>
                          <button className="act-btn" title="Send Email to This Customer" onClick={() => sendSingle(c.customer_id)} disabled={sendingId === c.customer_id}>
                            {sendingId === c.customer_id ? '…' : <Icon name="sendMail" size={14} />}
                          </button>
                          {c.status === 'DIFFERENCE' || c.soa_file
                            ? <button className="act-btn" title="Reconcile" onClick={() => onNavigate('recon', c.customer_id)}><Icon name="search" size={14} /></button>
                            : null}
                          {c.reupload_status === 'REQUESTED' && (
                            <button className="act-btn" title="Approve SOA re-upload request" onClick={() => approveReupload(c.customer_id)} style={{ color: 'var(--green)' }}><Icon name="approve" size={14} /></button>
                          )}
                          <button className="act-btn" title="Reset (for testing)" onClick={() => resetCustomer(c.customer_id)} disabled={resetting === c.customer_id}><Icon name="reset" size={13} /></button>
                        </div>
                      </td>
                    </tr>
                  );
          ...
          {emailResult && (
            <div className={`info-box ${emailResult.smtp_configured ? 'ib-green' : 'ib-amber'}`} style={{ marginTop: 16 }}>
              <strong><Icon name={emailResult.smtp_configured ? 'checkCircle' : 'warning'} size={13} /> {emailResult.smtp_configured ? 'Emails Sent' : 'SMTP Not Configured — Portal Links Logged'}</strong>
      ...
      {log.map((e, i) => (
        <div key={i} style={{ padding: '10px 0', borderBottom: '1px solid var(--border)', fontSize: 12 }}>
          ...
          {e.status === 'FAILED' && e.error && (
            <div style={{ marginTop: 4, fontSize: 10, color: '#C8102E', fontFamily: "'JetBrains Mono',monospace", wordBreak: 'break-word', display: 'flex', alignItems: 'flex-start', gap: 4 }}>
              <Icon name="warning" size={11} /> {e.error}
            </div>
          )}
        </div>
      ))}
